...
public class DefaultRestClient {
    ...
    public DefaultRestClient() throws IOException {
    ...
    public Configuration getConfiguration(String label, String name, String... profiles) {
        ...
        logger.info("Fetching config from server at: " + clientConfiguration.getUri());
        try {
            String[] labels = new String[]{""};
            if (StringUtils.hasText(label)) {
                labels = StringUtils.commaDelimitedListToStringArray(label);
            }
            // Try all the labels until one works
            for (String lbl : labels) {
                Configuration result = getRemoteConfiguration(restTemplate, clientConfiguration.getUri(), name, StringUtils.arrayToCommaDelimitedString(profiles), lbl.trim());
                if (result != null) {
                    logger.info(String.format("Located configuration: name=%s, profiles=%s, label=%s, version=%s",
                            result.getName(),
        ...
        if (clientConfiguration != null && clientConfiguration.isFailFast()) {
    ...
    private Configuration getRemoteConfiguration(RestTemplate restTemplate, String uri, String name, String profile, String label) {
        ...
        Configuration result = response.getBody();
        return result;
    }
    ...
    private RestTemplate getSecureRestTemplate(ClientConfiguration client) {
