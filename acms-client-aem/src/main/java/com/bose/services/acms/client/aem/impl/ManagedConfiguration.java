...
public class ManagedConfiguration {
     ...
     *
     * @param configurationProvider the {@see ConfigurationProvider} to use, not null.
     ...
     * @throws ManagedConfigurationException when configuring the node failed.
     ...
     * @see #configure(ConfigurationProvider, JcrSessionTemplate, boolean)
     */
    public boolean configure(ConfigurationProvider configurationProvider, JcrSessionTemplate sessionTemplate) throws ManagedConfigurationException, IllegalStateException {
        return configure(configurationProvider, sessionTemplate, false);
    }
     ...
     * {@see ManagedConfigurationMixin#PROPERTY_ADDITIONAL_PROFILES} property and pass them
     ...
     * @throws IllegalStateException         when the managed node can not be accessed. This could happen is the node gets deleted by another process.
     */
    public boolean configure(ConfigurationProvider configurationProvider, JcrSessionTemplate sessionTemplate, boolean refresh) throws ManagedConfigurationException, IllegalStateException {
        ...
        try {
            ...
            return (Boolean) sessionTemplate.executeWithResult(new JcrSessionTemplate.Callback() {
                ...
                public Object execute(Session session) throws Exception {
                    ...
                    if (node != null) {
                        ...
                        //todo, should re-read configuration name and profiles from node, because they could have changed...
                        //this means changing our API from mainly using node to using resources because of inheritance needs
                        if (!refresh || configurationProvider.refresh(configurationName, profiles)) {
                            Properties remoteProperties = configurationProvider.getProperties(configurationName, profiles);
            ...
            throw new ManagedConfigurationException("Error configuring managed node: " + ManagedConfiguration.this.toString(), e);
        }
